...
class Fiber_num_distribution():
    ...
    def fib_num_dis(self):
        ...
        fib_dia = []
        fib_dia_fra = []
        fib_dia = [465, 435, 405, 375, 345, 315, 285, 255, 225, 195, 165, 135, 105, 75, 45, 15] ## Fiber diameters
        if Fib_zone == 2:
            fib_dia_fra = [0.0033, 0.0000, 0.0000, 0.0000, 0.0000, 0.0033, 0.0016, 0.0341, 0.0927, 0.2813, 0.3545, 0.0927, 0.0146, 0.0293, 0.0846, 0.0081] ## Fiber diameter number distribution
    ...
    def fib_svf_dis(self):
        ...
        fib_dia = [465, 435, 405, 375, 345, 315, 285, 255, 225, 195, 165, 135, 105, 75, 45, 15] ## Fiber diameters
        if Fib_zone == 2:
            fib_dia_svf = [0.0230, 0.0000, 0.0000, 0.0000, 0.0000, 0.0105, 0.0043, 0.0725, 0.1533, 0.3495, 0.3153, 0.0552, 0.0053, 0.0054, 0.0056, 0.0001] ## Fiber diameter SVF distribution
        leng = len(fib_dia) ## Number of fiber diameters
        l = 0
        while l < leng:
...
class Bead_num_distribution():
    ...
    def Bead_num_dis(self):
        ...
        Bead_dia = []
        Bead_dia_fra = []
        Bead_dia = [465, 435, 405, 375, 345, 315, 285, 255, 225, 195, 165, 135, 105, 75, 45, 15] ## Fiber diameters
        if Bead_zone == 2:
            Bead_dia_fra = [0.0033, 0.0000, 0.0000, 0.0000, 0.0000, 0.0033, 0.0016, 0.0341, 0.0927, 0.2813, 0.3545, 0.0927, 0.0146, 0.0293, 0.0846, 0.0081] ## Fiber diameter number distribution
        leng = len(Bead_dia) ## Number of fiber diameters
        l = 0
        while l < leng:
    ...
    def Bead_svf_dis(self):
        ...
        Bead_dia = [465, 435, 405, 375, 345, 315, 285, 255, 225, 195, 165, 135, 105, 75, 45, 15] ## Fiber diameters
        if Bead_zone == 2:
            Bead_dia_svf = [0.0230, 0.0000, 0.0000, 0.0000, 0.0000, 0.0105, 0.0043, 0.0725, 0.1533, 0.3495, 0.3153, 0.0552, 0.0053, 0.0054, 0.0056, 0.0001] ## Fiber diameter SVF distribution
        leng = len(Bead_dia) ## Number of fiber diameters
        l = 0
        while l < leng:
...
coordinate_all = []                       # All the coordinates of fibers
bead_all = []                             # All the coordinates of beads
SV_all = 0                                # All the solid part volume of fibers
SVF_all = 0                               # FVF
SV_bd_all = 0                             # All the solid part volume of beads
SVF_bd_all = 0                            # BVF
...
Bead_num_all = 0                          # Number of all the beads
...
SVF_control_all = 0.01656                 # The SVF of fibers, FVF, this value can not be zero
SVF_bead_all = 0.1*SVF_control_all        # The SVF of beads, BVF
bead_lh_ratio = 2                         # Shape factor of the beads
bead_hf_ratio = 2                         # Size factor of the beads
SVF_control_factor_Fiber = 0.05           # Control precision of the FVF
SVF_control_factor_Bead = 0.1             # Control precision of the BVF
Fib_num_set = 67                          # Estimated total fiber numbers to generate
Bead_num_set = 44                         # Estimated total bead numbers to generate
Fib_zone = 2                              # Fiber generation zone, don't have specific meaning for this case, keep it as 2
Bead_zone = 2                             # Bead generation zone, don't have specific meaning for this case, keep it as 2
angle_control = 85                        # Control value of wind angle for each fiber
SVF_control_all_down = (1 - SVF_control_factor_Fiber)*SVF_control_all  # Control range of the FVF
SVF_control_all_up   = (1 + SVF_control_factor_Fiber)*SVF_control_all  # Control range of the FVF
SVF_bead_all_down = (1 - SVF_control_factor_Bead)*SVF_bead_all         # Control range of the BVF
SVF_bead_all_up   = (1 + SVF_control_factor_Bead)*SVF_bead_all         # Control range of the BVF
...
fib_dia_length = len(Fiber_num_dis)  # Total number of fiber diameters
...
Bead_dia_length = len(Bead_num_dis)  # Total number of bead diameters
dia_l = 0        # Control value of the fiber diamenter numbers
bead_l = 0       # Control value of the bead diamenter numbers
layer_gap = 0    # Control value of the distance between fibers and beads and the simulation zone boundary
...
if SVF_all < SVF_control_all:
    ...
    while dia_l < fib_dia_length:
        ...
        SVF_control = 0                                                 # SVF control value for the current fiber diameter
        SVF_control_Bead = 0                                            # SVF control value for the current bead diameter
        fiber_dia = Fiber_num_dis[dia_l][0]                             # Current fiber diameter
        bead_height = bead_hf_ratio * Bead_num_dis[bead_l][0]           # Current bead height
        bead_length = bead_lh_ratio * bead_height                       # Current bead length
        layer_gap = int(2 * fiber_dia / 2 / 2**0.5)                     # Gap control value for the current fiber diameter
        fiber_dia_num = int(Fiber_num_dis[dia_l][1])                    # Number of fibers to generate for current diameter
        bead_dia_num  = int(Bead_num_dis[bead_l][1])                    # Number of beads to generate for current diameter
        fiber_dia_svf = float(Fiber_svf_dis[dia_l][1]*SVF_control_all)  # SVF of the fibers for current diameter
        Bead_dia_svf = float(Bead_svf_dis[bead_l][1]*SVF_bead_all)      # SVF of the beads for current diameter
        SVF_control_factor_F = 0.5
        SVF_control_factor_B = 3
        SVF_control = fiber_dia_svf
        SVF_down = (1 - SVF_control_factor_F)*SVF_control               # Fiber SVF control range for current diameter
        SVF_up = (1 + SVF_control_factor_F)*SVF_control
        SVF_control_Bead = Bead_dia_svf
        SVF_down_Bead = (1 - SVF_control_factor_B)*SVF_control_Bead     # Bead SVF control range for current diameter
        ...
        Bead_num = 0                  # Number of beads generated for the current diameter
        ...
        Bead_num_all_b = Bead_num_all    # Backup of the number of beads generated until last diameter
        ...
        if SVF_judge <= SVF_control:
            ...
            while i < fiber_dia_num:
                ...
                Bead_num = len(bead)                                    # Number of beads generated of the current diameter
                Bead_num_all = len(bead_all)                            # Total number of beads generated
                    ...
                    if Fiber_num_all == 0:
                        ...
                        if bead_dia_num == 0 or bead_dia_num_temp == 0:
                            ...
                            if fib_dis_ok >= Fiber_num_all:                                
                                ...
                                i-= 1
                        elif bead_dia_num > 0 and bead_dia_num_temp > 0:
                            bead1 = [[0, 0, 0]]
                            bead1[0][0] = (codi1[0][0] + codi1[1][0]) / 2
                            bead1[0][1] = (codi1[0][1] + codi1[1][1]) / 2
                            bead1[0][2] = (codi1[0][2] + codi1[1][2]) / 2
                            ...
                            while jud_Bead < Bead_num_all:
                                ...
                                print("Bead_number_all_last = ",str(Bead_num_all_b), \
                                    ...
                                    "  Bead_number_dia = ",str(bead_dia_num), \
                                    "  Bead_number_now = ",str(Bead_num), \
                                    "  Bead_distance_judge = ",str(jud_Bead), \
                                    "  Bead_distance_ok = ",str(bead_dis_ok))
                                jud_Bead += 1
                                continue
...
if SVF_control_all_down <= SVF_all <= SVF_control_all_up and SVF_bead_all_down <= SVF_bd_all <= SVF_bead_all_up:
    ...
    print("SVF_all = ",SVF_all,"  Fiber_number_all = ",Fiber_num_all,"  SVF_bd_all = ",SVF_bd_all,"  Fead_number_all = ",Bead_num_all)
    print(*coordinate_all)
    print(*bead_all)
    ...
    f = open(file_name,'w')
    f.write("SVF_all = " + SVF_all + '\n' + \
            "BindAngle = " + str(windAngle_ave_all) + '\n' + \
            "Fiber_number = " + str(Fiber_num_all) + '\n' + \
            coordinate_all_str)
    f.close()
    file_name = str('All_bd.txt')
